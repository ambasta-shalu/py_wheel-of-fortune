...
class WOFPlayer:
    ...
    def __str__(self):
        return "{} ({})".format(self.name,self.prizes)
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        if self.prizeMoney != 0:
            ...
            if rand_letter != guessed:
                return rand_letter 
        else :
            VOWEL_COST = 250
            NOT_VOWELS = [letter for letter in 'BCDFGHJKLMNPQRSTVWXYZ']
            rand_letter = random.choice(NOT_VOWELS)
            if rand_letter != guessed:
                return rand_letter 
    ...
    def getMove(self,category, obscuredPhrase, guessed):
        ...
        if b == True:
            return max(SORTED_FREQUENCIES)
        else:
            return random.choice(SORTED_FREQUENCIES)



#sys.setExecutionLimit(100000) #  this take up to 10 minutes.
